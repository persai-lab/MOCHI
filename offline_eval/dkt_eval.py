...
# policies are built based on DKT plan model, which is used to trace student's knowledge based on
...
def eval_policy(dkt_agent, policy, traj_lengths, pretest_list, users_question_list,
    ...
    for traj, user_name in enumerate(users_name_list):
        ...
        dkt_agent.init_plan_model(pretest, top_k, problem_id_problem_name_mapping, reward_model)
        ...
        for t in range(traj_lengths[traj] if type(traj_lengths) == list else traj_lengths):
            ...
            true_qid = problem_name_problem_id_mapping[true_q]
            q_list.append(true_qid)
            a_list.append(true_a)
            dkt_agent.update_state(q_list, a_list)
        ...
        dkt_agent.update_reward()  # collect reward of curr. user into plan_model.finalRewards
    ...
    correlation_dch_simulated_reward = spearmanr(avg_dch_list, dkt_agent.finalRewards)
    ...
    correlation_ipw_dch_simulated_reward = spearmanr(avg_ipw_dch_list, dkt_agent.finalRewards)
    ...
    print("rewards  ", *np.round(dkt_agent.finalRewards, 4))
    return traj_lengths, avg_dch_list, avg_ipw_dch_list, traj_posttest_score_list, \
           traj_score_growth_list, dkt_agent.finalRewards, correlation_dch_post_score, \
...
if __name__ == '__main__':
    ...
    reward_model = "linear"
    print("offline evaluation policy: {}, top_k: {}, plan model: dkt".format(policy, top_k))
    ...
    user_records_dict = pickle.load(
        open("../data/MasteryGrids/user_records_dict.pkl", "rb"))
    ...
    problem_id_problem_name_mapping = pickle.load(
        open("../data/MasteryGrids/problem_id_problem_name_mapping.pkl", "rb")
    )
    ...
    # ------------------------------------------------------------------------------------
    # use all training users' records to train the DKT model and load the saved checkpoint
    arg_parser = argparse.ArgumentParser(description="DeepKT")
    arg_parser.add_argument('-c', '--config',
                            type=str, metavar='config_json_file',
                            default='dkt_masterygrids_fold_{}.json'.format(fold),
    ...
    dkt_agent = DKTAgent(config)
    dkt_agent.mode = "predict"
    # ------------------------------------------------------------------------------------
    if dkt_agent.mode == "test":
        # run student performance prediction on test dataset
        dkt_agent.run()
        dkt_agent.finalize()
    elif dkt_agent.mode == "predict":
        ...
        for index, user in enumerate(train_users_list):
            ...
            print("size: {}".format(len(q_list)))
            dkt_outputs = dkt_agent.predict(q_list, a_list)
            if len(q_list) >= dkt_agent.data_loader.max_seq_len:
                last_mastery = dkt_outputs[0, -1, :].tolist()
            else:
                init_mastery = dkt_outputs[0, 0, :]
                last_mastery = dkt_outputs[0, len(q_list), :].tolist()
                print("last mastery: {}".format(list(last_mastery)))
                # print("mastery change: {}".format(last_mastery - init_mastery))
                # print(dkt_outputs[0, len(q_list) - 1, :] - last_mastery)
                # print(dkt_outputs[0, len(q_list) + 1, :] - last_mastery)
                # print(dkt_outputs[0, len(q_list) + 2, :] - last_mastery)
            x = np.append([pretest], last_mastery)
            train_dkt_X.append(x)
            train_y.append(posttest)
            planRewardModel = LinearModel(train_dkt_X, train_y)
        ...
        for index, user in enumerate(users_list):
            ...
            print()
        if policy != "random":
            eval_policy(dkt_agent, policy, lenTraj, pretestList, users_q_list, users_a_list,
                        users_name_list, next_questions_dict, top_k, planRewardModel)
        else:
            traj_lengths_list = []
            avg_dch_list_list = []
            ...
            for i in range(100):
                outputs = eval_policy(dkt_agent, policy, lenTraj, pretestList, users_q_list,
                                      users_a_list, users_name_list, next_questions_dict, top_k)
